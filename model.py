...
class SummarizationModel(object):
  ...
  def _add_placeholders(self):
    ...
    # self._first_padding_mask=tf.placeholder(tf.float32, [hps.batch_size, hps.first_dec_steps], name='first_padding_mask')
    # self._first_batch = tf.placeholder(tf.int32, [hps.batch_size, hps.first_dec_steps], name='first_batch')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    # feed_dict[self._first_padding_mask] = batch.dec_padding_mask
    # feed_dict[self._first_batch] = batch.dec_batch
    if FLAGS.pointer_gen:
  ...
  def _concat_dim(self,d1_enc,d2_enc):
      ...
      with tf.variable_scope('concat_dim'):
          ...
          old_concat=tf.concat(axis=2,values=[d1_enc,d2_enc])#[batch_size, max_enc_steps, 5*hidden_dim]
          old_concat=tf.expand_dims(old_concat,axis=2)#[batch_size, max_enc_steps,1, 5*hidden_dim]
          new_concat=nn_ops.conv2d(old_concat, w_concat, [1, 1, 1, 1], "SAME")#[batch_size, max_enc_steps,1, hidden_dim * 2]
  ...
  def _add_decoder(self, inputs):
    ...
    # atten_gru=tf.contrib.rnn.GRUCell(hps.hidden_dim)
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      # Add the decoder.
      # with tf.variable_scope('firstpass_decoder'):
      #     self._first_state=self._add_firstpass_decoder(first_emb_dec_inputs)
        ...
        with tf.variable_scope('loss'):
          if FLAGS.pointer_gen:
            ...
            for dec_step, dist in enumerate(final_dists):
              ...
              #根据indices从dist中选出概率 如（0，1）从0中选择1的概率
