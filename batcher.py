...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.max_enc_seq_len2 = self.max_enc_seq_len/2 if self.max_enc_seq_len%2==0 else int(self.max_enc_seq_len/2)+1
    self.max_enc_seq_len3 = self.max_enc_seq_len2/2 if self.max_enc_seq_len2%2==0 else int(self.max_enc_seq_len2/2)+1
    ...
    self.enc_lens3 = np.zeros((hps.batch_size), dtype=np.int32)
    self.enc_padding_mask = np.zeros((hps.batch_size, max_enc_seq_len), dtype=np.float32)

    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.enc_batch[i, :] = ex.enc_input[:]#后面是数字
      self.enc_lens[i] = ex.enc_len #没加pad之前
      self.enc_lens2[i] = self.enc_lens[i]/2 if self.enc_lens[i]%2==0 else int(self.enc_lens[i]/2)+1
      self.enc_lens3[i] = self.enc_lens2[i]/2 if self.enc_lens2[i]%2==0 else int(self.enc_lens2[i]/2)+1
  ...
  def store_orig_strings(self, example_list):
    """Store the original article and abstract strings in the Batch object字符串"""
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (article, abstract) = next(input_gen) # read the next example from file. article and abstract are both strings.字符串
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        ...
        yield (article_text, abstract_text) #字符串
